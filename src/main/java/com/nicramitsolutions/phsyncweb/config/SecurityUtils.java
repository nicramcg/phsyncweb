...
public final class SecurityUtils {
    ...
    public static String getCurrentLogin() {
        ...
        logger.info("getCurrentLogin() security context + " + authentication);
        if (authentication != null) {
            if (authentication.getPrincipal() instanceof UserDetails) {
                springSecurityUser = (UserDetails) authentication.getPrincipal();
                userName = springSecurityUser.getUsername();
                logger.info("getCurrentLogin() security context + + first if: "  + userName);
            } else if (authentication.getPrincipal() instanceof String) {
                userName = (String) authentication.getPrincipal();
                logger.info("getCurrentLogin() + second if: "  + userName);
            }
        }
        logger.info("getCurrentLogin() + security context: "  + userName);
        return userName;
    }
    ...
    public static void sendError(HttpServletResponse response, Exception exception, int status, String message) throws IOException {
        ...
        com.nicramitsolutions.phsyncweb.config.Error error = new Error("authError", exception.getMessage());
